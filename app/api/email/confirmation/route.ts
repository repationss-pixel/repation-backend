...
function buildHtml(prenom: string, restaurantNom: string, creneau: string): string {
...
<head>
  ...
  <title>Confirmation de réservation Repation</title>
</head>
          ...
          <tr>
            <td style="background:#ffffff;border-radius:16px;box-shadow:0 2px 12px rgba(0,0,0,0.07);overflow:hidden;">
              ...
              <div style="background:#1D9E75;padding:28px 32px;">
                <p style="margin:0;font-size:22px;font-weight:700;color:#ffffff;">✅ Réservation confirmée !</p>
              </div>
...
export async function POST(req: NextRequest) {
  let body: Partial<ConfirmationEmailPayload>
  try {
    body = await req.json()
  } catch {
  ...
  const { data, error } = await resend.emails.send({
    ...
    to,
    subject: '✅ Votre réservation Repation est confirmée !',
    html: buildHtml(prenom, restaurantNom, creneau),
  })
